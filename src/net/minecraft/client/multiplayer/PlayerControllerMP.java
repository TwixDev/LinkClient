...
public class PlayerControllerMP {
    ...
    public float getBlockReachDistance() {
        return Autumn.MANAGER_REGISTRY.moduleManager.getModuleOrNull(TeleportMod.class).isEnabled() ? 200.0f : (this.currentGameType.isCreative() ? 5.0f : 4.5f);
    }
    ...
    public boolean sendBlockSword(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
        ...
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new C08PacketPlayerBlockPlacement(new BlockPos(-0.5423232, -0.5423232, -0.5423232), 255, this.mc.thePlayer.inventory.getCurrentItem(), 0.0f, 0.0f, 0.0f));
        int i = itemStackIn.stackSize;
        ItemStack itemstack = itemStackIn.useItemRightClick(worldIn, playerIn);
        if (itemstack != itemStackIn || itemstack != null && itemstack.stackSize != i) {
            playerIn.inventory.mainInventory[playerIn.inventory.currentItem] = itemstack;
