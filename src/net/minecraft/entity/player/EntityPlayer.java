...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	public EntityPlayer(World worldIn, GameProfile gameProfileIn) {
		...
		this.setLocationAndAngles((double) blockpos.getX() + 0.5, blockpos.getY() + 1, (double) blockpos.getZ() + 0.5,
				0.0f, 0.0f);
			...
			for (int i = 0; i < p_71010_2_; ++i) {
				...
				if (itemStackIn.getHasSubtypes()) {
					this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec31.xCoord, vec31.yCoord, vec31.zCoord,
							vec3.xCoord, vec3.yCoord + 0.05, vec3.zCoord, Item.getIdFromItem(itemStackIn.getItem()),
	...
	public void onLivingUpdate() {
		...
		if (this.getHealth() > 0.0f && !this.isSpectator()) {
			...
			axisalignedbb = this.ridingEntity != null && !this.ridingEntity.isDead
					? this.getEntityBoundingBox().union(this.ridingEntity.getEntityBoundingBox()).expand(1.0, 0.0, 1.0)
					: this.getEntityBoundingBox().expand(1.0, 0.5, 1.0);
	...
	private Collection<ScoreObjective> func_175137_e(Entity p_175137_1_) {
		...
		if (scoreplayerteam != null && (i = scoreplayerteam.getChatFormat().getColorIndex()) >= 0
				&& i < IScoreObjectiveCriteria.field_178793_i.length) {
		...
		if ((scoreplayerteam1 = this.getWorldScoreboard().getPlayersTeam(p_175137_1_.getName())) != null
				&& (j = scoreplayerteam1.getChatFormat().getColorIndex()) >= 0
				&& j < IScoreObjectiveCriteria.field_178792_h.length) {
	...
	public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
		...
		if (dropAround) {
			...
			float f2 = 0.3f;
			entityitem.motionX = -MathHelper.sin(this.rotationYaw / 180.0f * (float) Math.PI)
	...
	public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
		if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
			...
			int i = 0;
			float f1 = 0.0f;
			f1 = targetEntity instanceof EntityLivingBase
					? EnchantmentHelper.func_152377_a(this.getHeldItem(),
							((EntityLivingBase) targetEntity).getCreatureAttribute())
					: EnchantmentHelper.func_152377_a(this.getHeldItem(), EnumCreatureAttribute.UNDEFINED);
			...
			if (f > 0.0f || f1 > 0.0f) {
				boolean flag;
				boolean bl = flag = this.fallDistance > 0.0f && !this.onGround && !this.isOnLadder()
						&& !this.isInWater() && !this.isPotionActive(Potion.blindness) && this.ridingEntity == null
				...
				if (flag2) {
					...
					if (i > 0) {
						targetEntity.addVelocity(
								-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0f) * (float) i * 0.5f, 0.1,
					...
					if (targetEntity instanceof EntityDragonPart
							&& (ientitymultipart = ((EntityDragonPart) targetEntity).entityDragonObj) instanceof EntityLivingBase) {
	...
	public EnumStatus trySleep(BlockPos bedLocation) {
		if (!this.worldObj.isRemote) {
			...
			if (Math.abs(this.posX - (double) bedLocation.getX()) > 3.0
					|| Math.abs(this.posY - (double) bedLocation.getY()) > 2.0
					|| Math.abs(this.posZ - (double) bedLocation.getZ()) > 3.0) {
	...
	private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
		int i;
		if (this.ridingEntity != null && (i = Math.round(
			...
			if (this.ridingEntity instanceof EntityMinecart) {
				...
				if (this.startMinecartRidingCoordinate == null) {
					this.startMinecartRidingCoordinate = new BlockPos(this);
						...
						MathHelper.floor_double(this.posY), MathHelper.floor_double(this.posZ)) >= 1000000.0) {
	...
	public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
		...
		if (i >= 0 && i < this.inventory.armorInventory.length) {
			...
			if (itemStackIn != null && itemStackIn.getItem() != null
					&& (itemStackIn.getItem() instanceof ItemArmor ? EntityLiving.getArmorPosition(itemStackIn) != k
							: k != 4 || itemStackIn.getItem() != Items.skull
									&& !(itemStackIn.getItem() instanceof ItemBlock))) {
				return false;
			}
